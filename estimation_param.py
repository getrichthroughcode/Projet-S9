...
def estimate(X_pos):
    ...
    return (np.array(X_vit),np.array(X_acc)) #S.median_filter()

def MRU_param_estimation(X_pos,T):
    vit_est,acc_est = estimate(X_pos)
    corr = correlate(acc_est, acc_est, mode="full", method="fft") 
...
def MUA_param_estimation(X_pos,T):
    ...
    corr = correlate(jerk_est, jerk_est, mode="full", method="fft")
    
    len_dyn_pt_0 = np.linspace(0,len(jerk_est),len(jerk_est))
    len_dyn_pt_1 = np.linspace(len(jerk_est)-1,0,len(jerk_est)-1)
    len_dyn = np.concatenate((len_dyn_pt_0, len_dyn_pt_1))
    len_dyn[0]= 1e-8
    len_dyn[len(len_dyn)-1]= 1e-8
