...
def normalize_array(arr):
    ...
    """
    # Handle the case where max and min are the same (constant array)
...
def traj_choice(X,T):
    ...
    len_dyn_pt_0 = np.linspace(0,len(acc_est_x),len(acc_est_x))
    len_dyn_pt_1 = np.linspace(len(acc_est_x)-1,0,len(acc_est_x)-1)
    len_dyn = np.concatenate((len_dyn_pt_0, len_dyn_pt_1))
    len_dyn[0]= 1e-20
    len_dyn[len(len_dyn)-1]= 1e-20
    corr = correlate(acc_est_x, acc_est_x, mode="full", method="fft") / len_dyn
    corr_jerk = correlate(jerk_est_x, jerk_est_x, mode="full", method="fft") / len_dyn
    R_th_MRU = np.zeros(7)
    ...
    R_th_MRU[len(R_th_MRU) // 2 +2] = 1/T**4 * sigma_n
    R_th_MRU_norm = normalize_array(R_th_MRU)
    R_th_MUA = np.zeros(7)
    ...
    R_th_MUA_norm = normalize_array(R_th_MUA)
    corr_comp = corr[len(corr) // 2 -3 : len(corr) // 2+4]
    corr_comp_norm = normalize_array(corr_comp)
    error_MRU = np.sum(np.abs(corr_comp_norm-R_th_MRU_norm))
    
    corr_comp_j = corr_jerk[len(corr_jerk) // 2 -3 : len(corr_jerk) // 2+4]
    corr_comp_j_norm = normalize_array(corr_comp_j)
    error_MUA_j = np.sum(np.abs(corr_comp_j_norm-R_th_MUA_norm))

    plt.figure(figsize=(10, 6))
    plt.plot(corr_comp_j_norm,'o',label = "Générer jerk")
    plt.plot(R_th_MUA_norm,'x',label = "Théorique MUA")
    plt.legend()
    plt.show()
    plt.figure(figsize=(10, 6))
    plt.plot(corr_comp_norm,'o',label = "Générer")
    plt.plot(R_th_MUA_norm,'x',label = "Théorique MUA")
    plt.plot(R_th_MRU_norm,'x',label = "Théorique MRU")
    plt.legend()
    plt.show()
    print(np.abs(corr_comp_norm-R_th_MRU_norm))
    print(error_MRU)
    print(np.abs(corr_comp_j_norm-R_th_MUA_norm))
    print(error_MUA_j)
...
if __name__ == '__main__':
    ...
    SNR_list = [400,300,200,100,90, 80, 70, 60, 50, 40, 30, 20, 10, 9, 8, 7, 6, 5]
    ...
    for length in length_list:
        ...
        for SNR_id in range(len(SNR_list)):
            ...
            for i in range(N_moy):
                ...
                sigma = MUA_param_estimation(X_MUA[:,0] + bruit,T)
                

                erreur_est= (np.abs(sigma_th - sigma)/sigma_th)*100
                accu[l,SNR_id,i,:]= erreur_est
                inter_choice = traj_choice(X_MUA[:,0]+ bruit,T)
